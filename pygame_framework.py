...
class PygameDraw(b2DrawExtended):
    ...
    def DrawPolygonAA(self, vertices, color):
        """
        Draw a wireframe polygon given the screen vertices with the specified color.
        """
        if not vertices:
        ...
        for i in range(len(vertices)-1):
            pygame.draw.aalines(self.surface, color.bytes, tuple(vertices[i]), tuple(vertices[i+1]))

        pygame.draw.aalines(self.surface, color.bytes, tuple(vertices[-1]), tuple(vertices[0])) #close loop
    ...
    def to_physical_coord(self,point):
        x,y = point

        if self.flipX:
              x = self.screenSize.x - x

        xx = (x+self.offset.x)/self.zoom
...
class PygameFramework(FrameworkBase):
    ...
    def _Keyboard_Event(self, key, down=True):
        ...
        if down:
            if key==Keys.K_z:       # Zoom in
                self.viewZoom = min(1.1 * self.viewZoom, 2000.0)
            elif key==Keys.K_x:     # Zoom out
                self.viewZoom = max(0.9 * self.viewZoom, 1.00)
    ...
    def CheckKeys(self):
        ...
        if keys[Keys.K_LEFT]:
            self.viewCenter -= (0.5*s, 0)
        elif keys[Keys.K_RIGHT]:
            self.viewCenter += (0.5*s, 0)

        if keys[Keys.K_UP]:
            self.viewCenter += (0, 0.5*s)
